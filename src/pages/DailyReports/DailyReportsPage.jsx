...
const ALL_COLUMNS = [
  ...
  {
    key: "groceryTotal",
    header: "Grocery total",
    render: (r) => formatCurrency(r.groceryTotal),
  },
  {
    key: "volume",
    header: "Volume (Gallons)",
    render: (r) => formatNumber(r.volume),
  },
  {
    key: "cashDeposit",
    header: "Cash deposit",
    render: (r) => formatCurrency(r.cashDeposit),
  },
  {
    key: "checkDeposit",
    header: "Check deposit",
    render: (r) => formatCurrency(r.checkDeposit),
  },
  {
    key: "overShort",
    header: "Over / short",
    render: (r) => formatCurrency(r.overShort),
  },
  { key: "noSale", header: "No sale", render: (r) => formatNumber(r.noSale) },
  {
    key: "lineVoid",
    header: "Line void",
    render: (r) => formatNumber(r.lineVoid),
  },
  {
    key: "voidAmount",
    header: "Void amount",
  ...
  },
  {
    key: "refunds",
    header: "Refunds",
    render: (r) => formatCurrency(r.refunds),
  },
];
...
const sortByDate = (l, r) =>
  String(r.reportDate).localeCompare(String(l.reportDate));
...
const IconChevron = ({ open }) => (
  <svg
    ...
    strokeLinejoin="round"
    aria-hidden="true"
    className={`dr-filter-bar__chevron${open ? " dr-filter-bar__chevron--open" : ""}`}
  >
    <path d="M6 9l6 6 6-6" />
  </svg>
);
    ...
    try {
      ...
      reportsQuery.setData((current) =>
        !selectedReport
          ? [savedReport, ...(current || [])]
          : (current || []).map((r) =>
              r.dailyReportId === savedReport.dailyReportId ? savedReport : r,
            ),
      );
      notify({
        type: "success",
      ...
      <div className="dr-filter-bar">
        ...
        <div className={`dr-filter-bar__body${filterOpen ? " is-open" : ""}`}>
          ...
          <div className="dr-toolbar">
                ...
                <>
                  ...
                  <div className="dr-column-panel">
                    ...
                    <div className="dr-column-panel__actions">
                      <button
                        type="button"
                        className="dr-column-panel__action-btn"
                      ...
                      {filteredColumnOptions.map((col) => (
                        ...
                        >
                          <input
                            ...
                            disabled={col.sticky}
                          />
                          <span className="dr-column-panel__item-label">
                            {col.header}
                          </span>
                          {col.sticky && (
                            <span className="dr-column-panel__always">
                              Always
                            </span>
                          )}
                        </label>
            ...
            <button
              type="button"
              className="dr-toolbar-btn dr-toolbar-btn--ghost"
              ...
              <div className="dr-saved-views__list">
                {savedViews.map((view) => (
                  <span key={view.id} className="dr-saved-views__chip">
                    <button
                      type="button"
                      className="dr-saved-views__chip-name"
                ...
                {pageItems.map((report) => (
                  ...
                  >
                    <div className="dr-report-card__header">
                      <span className="dr-report-card__store">
                        {report.storeName}
                      </span>
                      <span className="dr-report-card__date">
                      ...
                      <div className="dr-report-card__metrics">
                        {metricColumns.map((col) => (
                          <div key={col.key} className="dr-report-card__metric">
                            <span className="dr-report-card__metric-label">
                              {col.header}
                            </span>
                            <span className="dr-report-card__metric-value">
                              {col.render(report)}
                            </span>
                          </div>
                        ))}
                      ...
                      {metricColumns.length > 0 && (
                        <div className="dr-report-card__metrics">
                          {metricColumns.slice(0, 3).map((col) => (
                            <div
                              key={col.key}
                              className="dr-report-card__metric"
                            >
                              <span className="dr-report-card__metric-label">
                                {col.header}
                              </span>
                ...
                {selectedReport ? (
                  <div className="dr-detail-panel">
                    <div className="dr-detail-panel__header">
                      <div>
                        <h3 className="dr-detail-panel__store">
                          {selectedReport.storeName}
                        </h3>
                        <span className="dr-detail-panel__date">
